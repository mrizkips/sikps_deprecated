...
class SidangController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($sidang)
                ...
                ->addColumn('action', function($row) {
                    $approve = view('components.sidang.approve', ['url' => route('baak.sidang.approval', $row->id), 'id' => $row->id]);
                    $disapprove = view('components.sidang.disapprove', ['url' => route('baak.sidang.approval', $row->id), 'id' => $row->id]);
                    $show = view('components.show', ['url' => route('baak.sidang.show', $row->id)]);
                    return $approve.$disapprove.$show;
                })
                ->rawColumns(['tipe', 'action'])
                ->make();
        }
        return view('baak.sidang.index');
    }
    ...
    {
        return view('baak.sidang.show', compact('sidang'));
    }
    ...
    public function approval(Request $request, Sidang $sidang, SidangService $service)
    {
        ...
        if ($service->approval($sidang, $fields)) {
            return redirect()->route('baak.sidang.index')->with('flash_messages', [
                'type' => 'success',
                'message' => $success,
            ]);
        }
